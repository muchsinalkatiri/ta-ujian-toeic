...
class Reading_comprehension extends CI_Controller {
	...
	{
		$data['page_title'] = 'Data Soal : Reading Comprehension';
		// Must login
		...
		$nama_part = 'Reading Comprehension';
	...
	public function tambah($nama_paket_id_part=null){
		$data['page_title'] = 'Tambah Data Soal : Reading Comprehension';
		if(!$this->session->userdata('logged_in') || $this->session->userdata('level') == '2' ){ 
			redirect('user/login/admin');
		}
		...
		if ($this->form_validation->run() == FALSE){
			...
			if ($nomer_soal < 153 || $nomer_soal >200) { //cek nomer soal bener ta gak
				$this->session->set_flashdata('nomer_soal_msg','nomer soal harus antara 153-200');    
				if($id_kelompok_soal == 0){redirect('admin/soal/reading_comprehension/tambah/'.$nama_paket.'4_5'.$id_part.'4_5'.$id_kelompok_soal);}
				else{redirect('admin/soal/reading_comprehension/tambah/'.$nama_paket.'4_5'.$id_part.'4_5'.$id_kelompok_soal);}
			}
				...
				if($check != 0){ //cek nomer soal di paket itu sudah ada apa belum
					...
					if ($insert) {
						...
						if($id_kelompok_soal == 0){redirect('admin/soal/reading_comprehension/data_soal/'.$nama_paket.'4_5'.$id_part);}
						else{redirect('admin/soal/reading_comprehension/data_kelompok_soal/'.$nama_paket.'4_5'.$id_part.'4_5'.$id_kelompok_soal);}
					}else{
						$this->session->set_flashdata('msg',
	...
	public function edit($id_soal = null){
		$data['page_title'] = 'Edit Data Soal : Reading Comprehension';
		if(!$this->session->userdata('logged_in') || $this->session->userdata('level') == '2' ){ 
			redirect('user/login/admin');
		}
		$nama_part = 'Reading Comprehension';
	...
	{
		$data['page_title'] = 'Data Kelompok Soal : Reading Comprehension';
		// Must login
		if(!$this->session->userdata('logged_in') || $this->session->userdata('level') == '2' ){ 
			redirect('user/login/admin');
		}
	...
	public function tambah_kelompok($nama_paket_id_part=null){
		...
		if ($insert) {
			...
			redirect('admin/soal/reading_comprehension/data_soal/'.$nama_paket.'4_5'.$id_part);
		}else{
			$this->session->set_flashdata('msg',
				'<div class="alert alert-danger">
